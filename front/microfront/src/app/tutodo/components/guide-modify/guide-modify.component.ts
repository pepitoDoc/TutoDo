...
export class GuideModifyComponent implements OnInit, OnDestroy {
  ...
  saveStep(index: number): void {
    ...
    this.stepsForm.controls.steps.controls[
      index
  ...
  modifyStep(index: number): void {
    ...
    this.stepsForm.controls.steps.controls[index].controls.saved.setValue(
      false
    );
  }
  ...
  saveGuideInfo(save: boolean): void {
    if (save) {
      ...
      this._service.saveGuideInfo$(payload).subscribe({
        next: (response) => {
  ...
  saveProgress(): void {
    const steps: Step[] = this.stepsForm.controls.steps.controls
      .filter((step) => step.valid)
      .map((step) => {
        return <Step>{
          title: step.controls.title.value,
          description: step.controls.description.value,
        };
      });
    ...
    this._service.saveGuideSteps$(payload).subscribe({
      next: (response) => {
        // Comprobar return
        this._toast.success(
          'Se ha guardado el progreso de la guía.',
          'Progreso guardado'
        );
      },
      error: (err) => {
  ...
  showInfo(): void {
    this._dialog.open(InfoDialogComponent, {
      ...
      data: {
        ...
        text: [
          'Para la creación de guías, deberá tener en cuenta lo siguiente:',
